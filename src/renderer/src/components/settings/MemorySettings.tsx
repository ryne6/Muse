...
import { Trash2, Search, Brain, Download, Upload, Pencil, Check, X, AlertTriangle, Loader2 } from 'lucide-react'
...
type CategoryFilter = 'all' | 'preference' | 'knowledge' | 'decision' | 'pattern'
...
export const MemorySettings = memo(function MemorySettings() {
  ...
  const [confirmClear, setConfirmClear] = useState<'user' | 'project' | null>(null)
  ...
  const loadMemories = useCallback(async () => {
    if (!memoryEnabled) { setLoadingInitial(false); return }
    try {
  ...
  const handleClearType = useCallback(async (type: 'user' | 'project') => {
    try {
      setLoading(true)
  ...
  const handleImport = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (data.length > MAX_IMPORT_ITEMS) {
        notify.error(`条目过多：最多支持 ${MAX_IMPORT_ITEMS} 条，当前 ${data.length} 条`)
        return
      }

      const result = await window.api.memory.import(data)
      notify.success(`导入 ${result.imported} 条，跳过 ${result.skipped} 条重复`)
      loadMemories()
    } catch {
  ...
  const filteredUser = useMemo(
    () => categoryFilter === 'all' ? userMemories : userMemories.filter((m) => m.category === categoryFilter),
    [userMemories, categoryFilter]
  )
  const filteredProject = useMemo(
    () => categoryFilter === 'all' ? projectMemories : projectMemories.filter((m) => m.category === categoryFilter),
    [projectMemories, categoryFilter]
  )
  const filteredSearch = useMemo(
    () => searchResults ? (categoryFilter === 'all' ? searchResults : searchResults.filter((m) => m.category === categoryFilter)) : null,
  ...
  const categoryColor = (cat: string) => {
    const map: Record<string, string> = {
      ...
      knowledge: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300',
      decision: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300',
      pattern: 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300',
    }
      ...
      >
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-xs px-1.5 py-0.5 rounded ${categoryColor(memory.category)}`}>
          ...
          {isEditing ? (
            <div className="flex items-center gap-1.5 mt-1">
              <Input
                ...
                autoFocus
              />
              <Button size="icon" variant="ghost" className="h-7 w-7 shrink-0" onClick={() => handleSaveEdit(memory.id)}>
        ...
        {!isEditing && (
          <div className="flex items-center gap-0.5 shrink-0">
            <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => handleEdit(memory)} disabled={loading}>
              <Pencil className="h-3.5 w-3.5" />
            </Button>
            <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => handleDelete(memory.id)} disabled={loading}>
    ...
    if (confirmClear === type) {
      return (
        <div className="flex items-center gap-1.5">
          ...
          <Button size="sm" variant="destructive" className="h-6 text-xs px-2" onClick={() => handleClearType(type)}>
            确认
          </Button>
          <Button size="sm" variant="ghost" className="h-6 text-xs px-2" onClick={() => setConfirmClear(null)}>
            取消
          </Button>
        </div>
      )
  ...
  const categories: CategoryFilter[] = ['all', 'preference', 'knowledge', 'decision', 'pattern']
      ...
      {memoryEnabled && (
        <>
          ...
          <div className="flex items-center justify-between text-xs text-muted-foreground border rounded-lg px-3 py-2">
            <span>共 {totalCount} 条记忆（用户 {userMemories.length} / 项目 {projectMemories.length}）</span>
            <div className="flex items-center gap-1.5">
              <Button size="sm" variant="ghost" className="h-7 text-xs gap-1" onClick={handleExport} disabled={loading}>
                <Download className="h-3.5 w-3.5" /> 导出
              </Button>
              <Button size="sm" variant="ghost" className="h-7 text-xs gap-1" onClick={() => fileInputRef.current?.click()} disabled={loading}>
          ...
          {searchResults && (
            <div className="space-y-3">
              ...
              {searchResults.length === 0 ? (
                <p className="text-sm text-muted-foreground py-4 text-center">无匹配结果</p>
              ) : (
